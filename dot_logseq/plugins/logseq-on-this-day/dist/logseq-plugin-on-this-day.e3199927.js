...
async function generateOTDPage(date, page) {
  ...
  if (journal_query_ret.length < 1) {
    ...
    const query_ret_pages = journal_query_ret === null || journal_query_ret === void 0 ? void 0 : journal_query_ret.flat();
...
async function getOnThisDay(showDate) {
  ...
  try {
    ...
    } else if (dateOnPage.getFullYear() == today.getFullYear && dateOnPage.getMonth() == today.getMonth() && dateOnPage.getDate() == today.getDate()) {
      return;
    } else {
      switch (showDate) {
        case "Previous":
    ...
    logseq.App.showMsg(err.message || "Maybe something wrong with the query", "error");
    console.log(err);
  }
}
async function jump(journalDay, showDate) {
  ...
  const journals = jump_query_ret === null || jump_query_ret === void 0 ? void 0 : jump_query_ret.flat();
  var tmpJournal;
  if (showDate == "Previous") {
