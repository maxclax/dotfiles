...
map_normal_mode("<leader>uf", require("utils.toggle").toggle_manual_folding, "Toggle manual folding")
...
function M.setup_snacks_keymaps()
	...
	return {
		...
		-- custom pickers
		{
			"<leader>sP",
			function()
				require("utils.snacks_pickers").pull_requests()
			end,
	...
	return {
		...
		},
		{
			...
			function()
				local default_branch = require("utils.git").get_default_branch()
				return vim.cmd(
					":DiffviewFileHistory --range=origin/"
						.. default_branch
...
function M.setup_terminal_keymaps()
	...
	return {
		{
			ctrl_alt_slash,
			require("utils.terminal").toggle_split_terminal,
			mode = { "n", "i", "t", "v" },
